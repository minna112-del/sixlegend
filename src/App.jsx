...
// WhatsApp group invite link (ঐচ্ছিক) — তোমাদের গ্রুপের Invite link দিলে “গ্রুপ খুলুন” বাটন কাজ করবে
...
const convertToBanglaNumber = (number) => {
  ...
  return Number(number || 0)
    .toFixed(2)
    .replace(/[0-9]/g, (x) => banglaNumbers[x]);
};
...
export default function App() {
  ...
  const handleAddExpense = async (e) => {
    ...
    const msg =
      `বাজার যোগ হয়েছে (${monthLabelBn(monthKey)})\n` +
      `পণ্য: ${payload.item}\n` +
      `দাম: $${Number(payload.amount).toFixed(2)}\n` +
      `কিনেছেন: ${payload.buyer}\n` +
      `সময়: ${new Date(ts).toLocaleString("bn-BD")}\n` +
  ...
  const balances = {};
  memberNamesOnly.forEach((name) => {
    balances[name] = memberSpending[name] - perPersonTotal[name]; // + = পাওনা, - = দেনা
  });
    ...
    return (
      <div className="min-h-screen bg-slate-200 flex justify-center items-center p-4">
        <div className="w-full max-w-[420px] bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="bg-[#1e3a8a] text-center py-6 px-4">
            <h1 className="text-yellow-400 text-3xl font-black mb-1 tracking-wider">লিল্লাহি এতিমখানা</h1>
            <p className="text-white/90 font-bold">প্রবেশ কোড দিন</p>
          </div>
          <div className="p-5 space-y-3">
            <input
              ...
              placeholder="কোড (1126)"
              className="w-full bg-slate-50 border p-4 rounded-2xl focus:outline-none focus:border-blue-500 text-center text-lg tracking-widest"
            />
            {pinError && <p className="text-red-600 text-sm font-semibold text-center">{pinError}</p>}
            <button onClick={unlock} className="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl">
              প্রবেশ করুন
            </button>
  ...
  const renderHeader = (title) => (
    <div className="bg-[#1e3a8a] text-white p-4 flex items-center gap-3 shadow-md sticky top-0 z-10">
      ...
      <button onClick={logout} className="bg-white/15 px-3 py-2 rounded-xl text-xs font-bold">
        Logout
      </button>
    </div>
  );
      ...
      <div className="bg-[#0b216b] p-4 text-white pb-safe mt-auto">
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-[#1e3a8a] rounded-xl py-4 text-center border border-blue-800 shadow-inner">
            ...
            <p className="text-[11px] text-blue-200 mt-1">{monthLabelBn(monthKey)}</p>
          </div>
          <div className="bg-[#1e3a8a] rounded-xl py-4 text-center border border-blue-800 shadow-inner">
      ...
      <div className="p-4 space-y-4 overflow-y-auto pb-24">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-3">
          ...
          {WHATSAPP_GROUP_INVITE_URL ? (
            <a href={WHATSAPP_GROUP_INVITE_URL} target="_blank" rel="noreferrer" className="bg-green-600 text-white font-bold px-4 py-3 rounded-xl text-sm">
              গ্রুপ খুলুন
            </a>
          ) : null}
        </div>
        ...
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-5 rounded-2xl shadow-lg text-white">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-blue-200 text-xs mb-1">বাসা ভাড়া</p>
            ...
            <div>
              <p className="text-blue-200 text-xs mb-1">ওয়াইফাই বিল</p>
                ...
                <div key={name} className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div>
                      ...
                      <p className={`font-black text-sm ${balance > 0 ? "text-green-700" : balance < 0 ? "text-red-700" : "text-slate-600"}`}>
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            <p className="text-center text-slate-500 py-4">লোড হচ্ছে...</p>
          ) : marketItems.length === 0 ? (
            <p className="text-center text-slate-400 py-4">এই মাসে কোনো খরচ যুক্ত করা হয়নি</p>
          ) : (
              ...
              <div key={item.id} className="bg-white p-3 rounded-xl border flex justify-between items-center shadow-sm">
                {editingId === item.id ? (
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleSaveEdit(item.id)} className="flex-1 bg-green-600 text-white py-2 rounded-lg text-sm font-bold">
                        সেভ করুন
                      </button>
                      <button onClick={handleCancelEdit} className="flex-1 bg-slate-400 text-white py-2 rounded-lg text-sm font-bold">
                        বাতিল
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => handleEditExpense(item)} className="text-blue-500 p-1">
                        <EditIcon />
                      </button>
                      <button onClick={() => handleDeleteExpense(item.id)} className="text-slate-400 hover:text-red-500 p-1">
                        <TrashIcon />
                      </button>
                    </div>
                  </>
      ...
      {shareOpen && (
        <div className="absolute inset-0 bg-black/40 flex items-end sm:items-center justify-center p-4">
          <div className="w-full max-w-[420px] bg-white rounded-2xl p-4 shadow-2xl">
            ...
            <textarea readOnly value={shareText} className="w-full h-32 bg-slate-50 border rounded-xl p-3 text-sm" />
            <div className="flex gap-2 mt-3">
              <button
                onClick={() => window.open(makeWhatsAppShareUrl(shareText), "_blank")}
                className="flex-1 bg-green-600 text-white font-bold py-3 rounded-xl"
              >
                WhatsApp-এ শেয়ার
              </button>
              <button onClick={() => setShareOpen(false)} className="flex-1 bg-slate-200 text-slate-800 font-bold py-3 rounded-xl">
                পরে করবো
              </button>
            </div>
            {WHATSAPP_GROUP_INVITE_URL ? (
              <a href={WHATSAPP_GROUP_INVITE_URL} target="_blank" rel="noreferrer" className="block mt-2 text-center text-sm font-bold text-green-700">
                গ্রুপ খুলুন
              </a>
            ) : null}
          </div>
  ...
  const renderSchedule = (type) => {
    ...
    const scheduleData = MEMBERS.map((m) => ({
      ...m,
      isToday: isCook ? memberNamesOnly[getCookIndex(today)] === m.name : memberNamesOnly[getCleanerIndex(today)] === m.name,
    }));
